...
struct AutoInitializer {
    AutoInitializer() {
        ...
        try {
            ...
            log_message("Initializing GGML visualization");
            log_message("  Output file: " + std::string(output_file));
            log_message("  Configuration loaded from environment variables");

            // Initialize the hook - ConfigManager will automatically load settings from:
            // 1. GGML_VIZ_CONFIG environment variable (if set)
            // 2. Environment variable overrides (GGML_VIZ_OP_TIMING, etc.)
            // 3. Default values
            // The Config::apply_env_overrides() method handles all legacy env vars
